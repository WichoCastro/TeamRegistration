<?
	...
	if ($_SESSION['logged_in']) {
		...
		print $banner;
		print $navBar;
		print "<div id='ttlBar'>Change Log</div>";
		print "<form name='frm' id='frm'>";print "<table>";
		print "<tr><td>Show change log for:</td><td><input type='text' value='$start_date' name='start_date' id='start_date'>";
		...
		if ($start_date) {
			...
			$sql="SELECT action_date, name, action, tbl, whr, setClause FROM tmsl_change_log c INNER JOIN tmsl_user u ON c.user_uid=u.player_uid";
			$sql.=" WHERE action_date >= STR_TO_DATE('$start_date', '%m/%d/%Y') AND action_date < STR_TO_DATE('$stop_date', '%m/%d/%Y')";
