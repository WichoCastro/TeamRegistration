<?
	...
	if ($_SESSION['logged_in']) {
		if (!$start_date) $start_date = date('m/d/Y', time()-30*24*3600);
		print "<html>";
		print "<head>";
		...
		print $banner;
		print $navBar;
		print "<div id='ttlBar'>PayPal Log</div>";
		print "<form name='frm' id='frm'>";print "<table>";
		print "<tr><td>Show transactions starting from:</td><td><input type='text' value='$start_date' name='start_date' id='start_date'>";
		...
		if ($start_date) {
			...
			$stop_date=date('m/d/Y',mktime(0,0,0,$mm+1,$dd,$yyyy));
			$sql="SELECT dt, lname, fname, amt FROM tmsl_pp PP INNER JOIN tmsl_player P ON PP.user_id = P.uid";
			$sql.=" WHERE dt >= STR_TO_DATE('$start_date', '%m/%d/%Y') AND dt < STR_TO_DATE('$stop_date', '%m/%d/%Y')";
