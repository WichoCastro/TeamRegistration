            ...
            if ($commit) {
                foreach ($team_id as $tid) {
                    if (hasPermission($_SESSION['mask'], $tid, $season_id)) {
                        ...
                        $sql = "UPDATE tmsl_player_team SET registered = 0 WHERE team_uid = $tid AND season_uid=$season_id";
                        mysql_query($sql);
                        ...
                        $sql = "UPDATE tmsl_team_season SET registered = 0 WHERE team_uid = $tid AND season_uid=$season_id";
                        mysql_query($sql);
                    }
                }
